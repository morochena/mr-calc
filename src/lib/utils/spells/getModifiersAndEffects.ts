import type {
	...
	CombinedModifier,
	ProcessedModifier,
	CombinedModifierOrEffect,
	ProcessedEffect,
	ProcessedModifierOrEffect,
	Effect
} from '../../../../types/types';
...
export function getProcessedEffects(spell: Spell) {
	const processedEffects = cloneDeep(getCombinedEffects(spell)) as ProcessedEffect[];
	...
	if (spell.mode === 'Unpredictable') {
		processedEffects.push({
			...
			amount: 3,
			description:
				'Double the value of the dice rolled when the spell is cast. When the lowest die is a 5, add 10 instead, when the lowest die is 8 add 16, etc.',
			domainTier: 1,
			fromDomain: true
		});
	}
	...
	switch (spell.domain) {
		case 'Fire':
			...
			break;
		case 'Air':
			const air = processedEffects.find((air) => air.id === createElementId);
			if (!air) {
				processedEffects.push({
					id: 0,
			...
			break;
		case 'Water':
			const water = processedEffects.find((water) => water.id === createElementId);
			if (!water) {
				processedEffects.push({
					id: 0,
					name: 'Require Element',
			...
			break;
		case 'Earth':
			const force = processedEffects.find((force) => force.id === applyForceId);
			...
			const earth = processedEffects.find((earth) => earth.name === 'Create Element');
			if (!earth) {
				processedEffects.push({
					id: 0,
					name: 'Require Element',
...
export function getProcessedModifiers(spell: Spell) {
	const modifiers = getCombinedModifiers(spell) as ProcessedModifier[];
...
function processFireDomain(
	processedEffects: ProcessedEffect[],
	damageId: number,
	damageEffect: Effect,
	lightId: number,
	lightEffect: Effect,
	createElementId: number
) {
	const x = processedEffects.find((x) => x.id === damageId);
	...
	const y = processedEffects.find((y) => y.id === lightId);
	...
	const z = processedEffects.find((z) => z.id === createElementId);
	if (!z) {
		processedEffects.push({
			id: 0,
			name: 'Require Element',
...
function calcIllusionDiscount(spell: Spell, effect: CombinedEffect, trueTier?: number) {
	...
	const helpEffects = effects.filter((x) => x.name.includes('Help'));
	let helpSP = 0;
	...
	const unhelpEffects = getCombinedEffects(spell).filter(
